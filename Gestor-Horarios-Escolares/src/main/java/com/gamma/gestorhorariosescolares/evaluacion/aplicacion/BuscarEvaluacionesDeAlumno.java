...
public class BuscarEvaluacionesDeAlumno {
    ...
    public BuscarEvaluacionesDeAlumno(ServicioBuscador<Inscripcion> buscadorInscripcion, ServicioBuscador<Materia> buscadorMateria,
                                      ServicioBuscador<Maestro> buscadorMaestro, ServicioBuscador<Evaluacion> buscadorEvaluacion) {
    ...
    public EvaluacionesData buscarEvaluaciones(Integer periodoEscolar, Integer idAlumno) throws RecursoNoEncontradoException {
        Inscripcion inscripcion = buscadorInscripcion
                .igual("idPeriodoEscolar", periodoEscolar.toString())
                .igual("idAlumno", idAlumno.toString())
                .buscarPrimero();

        List<Evaluacion> evaluaciones = buscadorEvaluacion
                .igual("idInscripcion", String.valueOf(inscripcion.id()))
                .buscar();

        List<EvaluacionData> evaluacionesData = new ArrayList<>();
        for (Evaluacion evaluacion : evaluaciones) {
            Materia materia = buscadorMateria
                    .igual("id", evaluacion.idMateria().toString())
                    .buscarPrimero();
            Maestro maestro = buscadorMaestro
                    .igual("id", evaluacion.idMaestro().toString())
                    .buscarPrimero();
